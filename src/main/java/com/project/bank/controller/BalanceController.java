...
import com.project.bank.model.Balance;
import com.project.bank.service.BalanceService;
import com.project.bank.service.BalanceServiceImpl;

public class BalanceController {
	
	BalanceService bankService = new BalanceServiceImpl();
	...
	public void performTransaction(Scanner input, String transaction_type) {
		...
		if (isValidAccount(account_id)) {
			...
			if (transaction_type.equals("deposit")) {
				...
				bankService.depositAmount(balance);
				...
				if (hasBalance(account_id, withdraw_amount)) {
					...
					bankService.withdrawAmount(balance);
	...
	public boolean isValidAccount(int account_id) {
		return bankService.getAccount(account_id);
	}
	...
	public void getBalance(Scanner input) {
		...
		System.out.println("Your balance is : $" + bs.getBalance(account_id));
	}
